...
<div class="tip">
	...
	<p>您的好友分享一款酷炫产品${ tGoods.goodsname}邀请您0元领取，邀请好友还可以获得奖励喔，快来领取吧！</p>
	<%--<p>您的好友(${sharephone.replace(sharephone,sharephone.replace(sharephone.substring(3,7),"****")) })邀请您0元领取一款酷炫产品，邀请好友还可以获得奖励喔，快来领取吧！</p>--%>
</div>
    </c:if>
<!-- 弹框 -->
<c:if test="${not empty bActivitySharing}">
<div class="pop-up" >
	...
	<div class="pop-up-box open-red">
		...
		<div class="red-box <c:if test="${empty bActivitySharing.besharedRewardValue or bActivitySharing.besharedRewardValue == 0}">zero</c:if>">
			...
			<strong>&nbsp;
				<c:if test="${bActivitySharing.besharedRewardValue or bActivitySharing.besharedRewardValue != 0}">
			...
			<!-- 领取完成-->
			<div class="received">
				<p>已经放入您的账户(<font id="sharedPhone">180***1234</font>)中，您可前往“个人中心-我的收入”中查看使用</p>
		...
		<div class="red-box">
			...
			<div class="input-box">
				...
				<div class="code-box">
					...
					<span id="logingetCode" class="get-code">获取</span>
				</div>
	...
	<div class="pop-up-box share-box">
		<div class="title">
			<p>分享有礼</p>
			<a class="rule" href="/share/ruleDetails?money=${bActivitySharing.shareRewardValue}">规则详情</a>
		</div>
		<div class="intro">
			<span></span>

			<p>
                本商品参与分享有礼现金奖励活动，分享好友并安装成功，您将获得${bActivitySharing.shareRewardValue}元现金奖励。同时，您的好友也可以参与分享有礼活动。
...
<input type="hidden"  id="useridflag" value="" />

<input type="hidden" id="autoLoginUserId" value="${autoLoginUserId}">
...
<div class="store-list">
    ...
    <c:forEach var="cfacilitator" items="${tgList}">
        <ul>
            <li>
...
<script>
	...
	$("#openBtn").click(function(){
        ...
        //自动领取
        var autoLoginUserId = $("#autoLoginUserId").val();
        if(autoLoginUserId != ''){
            var shareuserid = $("#shareuserid").val();
            var goodsid = $("#goodsid").val();
            $.post("/shared/getshare.do",
                {userid:autoLoginUserId,goodsid:goodsid,shareuserid:shareuserid},
                function (msg) {
                    if(msg.status == 1){
                        ...
                        $('.pop-up').show();
                        $(".open-red").show();
                        $(".open-red .input-box").hide();
                        $(".open-red .received").show();
    ...
    $('.open-share').click(function () {
        if($("#useridflag").val() != null && $("#useridflag").val() != ''){
            popHide();
            $('.pop-up').show();
            $('.share-box').show();
        }else{
            ...
            var autoLoginUserId = $("#autoLoginUserId").val();
            if(autoLoginUserId != ''){
                $("#useridflag").val(autoLoginUserId);
                $('.pop-up').show();
                $('.share-box').show();
            }else{
                $('.pop-up').show();
