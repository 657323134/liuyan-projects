...
$('#popWindows').on("click","#receive",function () {
    $('.pop-up').show();
    $('.open-red').show();
});
        ...
        function (msg) {
            if (msg.status == 1) {
                ...
                $.post("/shared/verifycuser",
                    {phone: loginphone, openid: openid,shareuserid: $("shareuserid").val()},
                    function (msg) {
...
(function(){
    ...
    $.ajax({
        url:"/share/loadSharedConfig2?" + urlId +"&page=" + encodeURIComponent(asb),
        type: "get",
        dataType:"json",
        success:function(data){
            if(data.code== 200){
                ...
                template.helper('getObject', function () {
                    return {jquery:$,toFixed:toFixed,replace:replace,substring:substring};
                });
                ...
                //load
                if( data.data.bActivitySharing == undefined || data.data.bActivitySharing == null || data.data.bActivitySharing == ""){
...
//tips点击领取
$('#hiddenValues').on("click",".openReceiveBtn",function () {
    popHide();
    $('.pop-up').show();
    $('.receive-red').show();
});
...
//领取红包-验证手机号领取。
$('#popWindows').on("click","#openBtn",function () {
    popHide();
