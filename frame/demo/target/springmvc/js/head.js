...
function fun_addtradepwd() {
    ...
    if (confirmTradePwd == tradePwd) {
        fn_ajax("/member/setTradePwd", {
            tradePwd: $.md5(tradePwd),
            confirmTradePwd: $.md5(confirmTradePwd)
        }, function (data) {
            if (data == "1") {
...
function show_weixin() {
    ...
    content += "	  <h2 style=\"float:left;\">关注微品金融官方微信</h2>";
    ...
    content += "	    <p style=\"padding:7px 10px 0 10px;font-size:12px;line-height:20px;\">打开微信，点击底部的″发现″，使用″扫一扫″即可关注微品金融官方微信。</p>";
    content += "	  </div>";
    content += "	</div>";
    content += "</div>";
    content += "</div>";
