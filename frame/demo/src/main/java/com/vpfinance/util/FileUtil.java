...
public class FileUtil {
     ...
     * @param path    保存路径（父级文件夹可以不存在）以'/'结尾
     * @param types   可上传类型
     * @param map     KEY：上传文件的name,VALUE:保存文件名，不包括后缀
    ...
    public static List<String> uploads(HttpServletRequest request, String fileName,
        ...
        try {
            ...
            for (MultipartFile file : filelist) {
                ...
                //com.vpfinance.util.ImageCompressUtil.saveMinPhoto(files.getAbsolutePath(), files.getAbsolutePath(), 8000, 0.5d);//文件缩小
     ...
     * @author ransheng
     */
    public static String uploadmultiple(HttpServletRequest request, MultipartFile file, String fileName,
                                        String folder) {
        try {
            ...
            String str = random.nextInt(9999) + new SimpleDateFormat("yyyyMMddHHmmss")
    ...
    public static String upload(HttpServletRequest request, String fileName,
                                String folder, MultipartFile file) {
        try {
//            MultipartFile file = null;
//            MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request;
//            file = multipartRequest.getFile(fileName);
            ...
            Properties p = PropertiesUtil.getReadAbleProperties("/sys.properties");
            String root = p.getProperty("FILE_PATH");
            // 取得根目录
            root += "/" + folder + "/";
            System.out.println(root);
            if (!createFolder(root)) {
...
//                    + ":"
//                    + request.getServerPort()
//                    + (request.getContextPath() + "/imgs/" + folder + "/" + str)
//                            .replace("//", "/");
            String imgurl = "/" + folder + "/" + str;
            //com.vpfinance.util.ImageCompressUtil.saveMinPhoto(files.getAbsolutePath(), files.getAbsolutePath(), 8000, 0.5d);//文件缩小
...
//            ImageCompressUtil.saveMinPhoto(files.getAbsolutePath(), files.getAbsolutePath(), 8000, 0.5d);//文件缩小
    ...
    public static String getImageHost() {
        return PropertiesUtils.getProperties("img_url");
    }

    public static void deleteFile(String path, String disk) {
        String absolutePath = PropertiesUtils.getSysProperties("FILE_PATH") + path;
